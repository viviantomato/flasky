...
def create_one_menu():
    ...
    new_menu = Menu(restaurant_name = request_body.get("restaurant_name"), meal = request_body.get("meal"))
...
def delete_menu(menu_id):
    ...
    db.session.commit()
    return jsonify({"msg": f"Menu with id {menu.id} was successfully deleted."}), 200